...
PRINT '| Modified Date: December 17, 2021  |'
PRINT '+-----------------------------------+'
...
/*
eStore Triggers - eStore Database:
=================================
*/
-- 1. Create a trigger called TR1 that will enforce the rule that a rule that selling price of a new product cannot
--    exceed 150% of the order cost.


-- 2. Create a trigger called TR2 that will enforce the rule that the selling price of a product cannot increase by
--    more than 5%.


-- 3. Products in the inventory need to be audited for price changes. Create a single trigger called TR3 that will
--    track the price change on a product. You will need to create a table called ProductAudit that will
--    have the following definition:

CREATE TABLE ProductAudit
(
    ProductAuditID INT IDENTITY(1,1) NOT NULL
		CONSTRAINT PK_ProductAudit_AuditID PRIMARY KEY CLUSTERED,
	OldSellingPrice DECIMAL(8,2) NOT NULL,
	NewSellingPrice DECIMAL(8,2) NOT NULL,
	PriceDifference DECIMAL(8,2) NOT NULL, -- can be negative, but not zero
	ProductID INT NOT NULL,
	ChangedOn DATETIME NOT NULL
		CONSTRAINT DF_ProductAudit_ChangedOn_GetdDate DEFAULT (GETDATE())  
)
GO
	
-- 4. Create a trigger called TR4 that will prevent the removal of a SalesOrderDetail record.


-- 5. Create a trigger for a new Product that if the quantity on hand is zero, set the on order to the
--    reorder level plus 10%.
